import matplotlib.pyplot as plt
from model import *
# 加载最佳模型
model.load_weights("best_model.h5")

# 使用动态滚动预测
def rolling_forecast(model, initial_data, steps):
    predictions = []
    current_batch = initial_data.reshape(1, initial_data.shape[0], 1)
    for _ in range(steps):
        current_pred = model.predict(current_batch)[0]
        predictions.append(current_pred[0])  # 取第一个预测步长
        # 更新batch：移除最早点，添加预测值
        current_batch = dg.np.append(current_batch[:,1:,:], [[current_pred[0]]], axis=1)
    return predictions
# 使用最后60个训练数据作为初始状态
last_train_window = dg.scaled_train[-60:]
y_pred_rolled = rolling_forecast(model, last_train_window, len(dg.y_test))

# 测试集预测
y_pred = model.predict(dg.X_test)
y_pred = dg.scaler.inverse_transform(y_pred)  # 反归一化
y_true = dg.scaler.inverse_transform(dg.y_test.reshape(-1,1))

# 结果可视化 [4,5](@ref)
plt.figure(figsize=(14,6))
plt.plot(y_true, label='Actual Price', color='blue', alpha=0.6)
plt.plot(y_pred, label='Predicted Price', color='red', linestyle='--')
plt.title('PingAn Bank Stock Price Prediction')
plt.xlabel('Trading Days')
plt.ylabel('Price (CNY)')
plt.legend()
plt.grid(True)
plt.show()

# 与原预测方法对比
plt.plot(y_true, label='Actual')
plt.plot(y_pred, label='Static')
plt.plot(y_pred_rolled, label='Rolling')
plt.legend()

# 评估指标
from sklearn.metrics import mean_squared_error, mean_absolute_error
mse = mean_squared_error(y_true, y_pred)
mae = mean_absolute_error(y_true, y_pred)
print(f"Test MSE: {mse:.4f}, MAE: {mae:.4f}")